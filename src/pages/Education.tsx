...
const Education: React.FC = () => {
  ...
  useEffect(() => {
    const loadContent = async () => {
      try {
        setIsLoading(true);
        const fetchedContent = await firestoreService.getEducationalContent();
        ...
        if (fetchedContent.length > 0) {
          ...
          // Upload all mock content
          await Promise.all(mockContent.map(async (article) => {
            const { id, ...contentData } = article;
            return firestoreService.addEducationalContent(contentData);
          }));

          // Fetch the newly added content
          const newContent = await firestoreService.getEducationalContent();
        ...
        addNotification({
          title: 'Error',
          message: 'Failed to load educational resources',
          type: 'error'
        });
      } finally {
        setIsLoading(false);
